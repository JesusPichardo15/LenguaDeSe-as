        ...
        {anim.map((animal, index) => (
          <li key={index} className={`p-4 text-center bg-white rounded-lg shadow-lg transition-transform transform hover:scale-105`}>
            <img 
              src={animal.img} 
              alt={animal.name} 
              className={`w-full h-96 border-4 object-contain rounded mb-2 transition duration-300 border-[${animal.hoverColor}]`} // Borde según el color de hover
            />
            <span className="font-bold text-[#fe6622]">{animal.name}</span> {/* Texto en color naranja */}
          </li>
        ))}
      </ul>
    </div>
